...
def compute_adversarial_gap(
    ...
    if noise_type == "gaussian":
        ...
        noise = (
            np.random.uniform(-1, 1, (n_samples, n_features))
            * noise_scale
            * embedding_std
        )
    else:
        raise ValueError(
            f"Unknown noise_type: {noise_type}. Use 'gaussian' or 'uniform'"
        )
...
def _self_test():
    ...
    gap_uniform = compute_adversarial_gap(
        embeddings, noise_scale=0.1, noise_type="uniform", seed=42
    )
