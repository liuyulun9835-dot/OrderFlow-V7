...
def compute_clarity_spectrum_power(
    ...
    frequencies, psd = signal.welch(
        clarity, fs=sampling_rate, nperseg=nperseg, scaling="density"
    )
...
def _self_test():
    ...
    clarity = 0.7 + 0.1 * np.sin(
        2 * np.pi * 35.0 * t
    )  # 35 Hz oscillation (in upper half)
    power = compute_clarity_spectrum_power(clarity, sampling_rate=100.0)
    assert (
        power > 0.0001
    ), "Fast oscillation should have significant high-frequency power"
    ...
    power = compute_clarity_spectrum_power(
        clarity, sampling_rate=100.0, frequency_band=(4.0, 6.0)  # Band around 5 Hz
    )
